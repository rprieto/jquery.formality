...
You can think of it as the counterpart to templating (for example using <a href="https://github.com/janl/mustache.js/">Mustache</a> or <a href="https://github.com/jquery/jquery-tmpl">jQuery Template</a>). This can help implement MVVM-like models by simplifying the two-way mapping between the view and the presentation layer.
...
_Formality_ can also create a hierarchical representation of a nested form by adding a few attributes to the HTML. The main uses cases are probably:

* Creating a javascript object that can be used by the presenter or viewmodel, or that maps directly to your domain model.
* Handling the case where user creates several items on the front end, which we intend on sending to the server in bulk using ajax.
...
The basic case for using *data-formality-nested* is to group elements logically.
